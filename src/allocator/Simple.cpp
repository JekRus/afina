...
Simple::Simple(void *base, size_t size) : _base(base), _base_len(size) {
  Node first_node{size - sizeof(List) - sizeof(int) - 2 * sizeof(Descriptor),
                  nullptr};
...
Pointer Simple::alloc(size_t N) {
  void *server_p = static_cast<void *>(
      static_cast<uint8_t *>(_base) + (_base_len - sizeof(List) - sizeof(int)));

  Descriptor *descriptor_addr;
  List *lst_p = reinterpret_cast<List *>(static_cast<uint8_t *>(_base) +
                                         (_base_len - sizeof(List)));
  if (N < sizeof(Node)) {
    N = sizeof(Node);
  }
  ...
  if (reinterpret_cast<List *>(p) == lst_p) {
    ...
    descriptor_addr =
        descriptor.add_descriptor(static_cast<int *>(server_p), is_new_space);
    auto old_node_size = head->size;
    if (old_node_size >= sizeof(Node) + N) {
      Node *new_node_p =
          reinterpret_cast<Node *>(reinterpret_cast<uint8_t *>(head) + N);
      lst_p->delete_node(nullptr, true);
      new_node_p->next = head->next;
      new_node_p->size = old_node_size - N;
      lst_p->head = new_node_p;
    } else {
      lst_p->delete_node(nullptr, true);
    }
  } else {
    ...
    descriptor_addr =
        descriptor.add_descriptor(static_cast<int *>(server_p), is_new_space);
    if (old_node_size >= sizeof(Node) + N) {
      Node *new_node_p = reinterpret_cast<Node *>(
          reinterpret_cast<uint8_t *>(node_p->next) + N);
  ...
  if (is_new_space && (lst_p->head != nullptr)) {
    auto q = lst_p->head;
    if (q->next == nullptr) {
...
void Simple::realloc(Pointer &p, size_t N) {
  ...
  if (mem_end_ptr == static_cast<void *>(head)) {
    if (head->size + old_size >= N + sizeof(Node)) {
      ...
      head = reinterpret_cast<Node *>(reinterpret_cast<uint8_t *>(head) -
                                      old_size + N);
      *head = new_node;
      return;
    ...
    while ((node_p->next != nullptr) &&
           (static_cast<void *>(node_p->next) != mem_end_ptr)) {
      node_p = node_p->next;
    }
    if (static_cast<void *>(node_p->next) == mem_end_ptr) {
      if (node_p->next->size + old_size >= N + sizeof(Node)) {
        ...
        node_p->next = reinterpret_cast<Node *>(
            reinterpret_cast<uint8_t *>(node_p->next) - old_size + N);
  ...
  if ((head == nullptr) || (static_cast<Node *>(descriptor_ptr) < head)) {
    ...
    *(static_cast<Node *>(descriptor_ptr)) = Node{p.getsize(), nullptr};
    ...
    while ((q->next != nullptr) &&
           (q->next < static_cast<Node *>(descriptor_ptr))) {
      q = q->next;
    }
...
void Simple::defrag() {
  ...
  while (head->next != nullptr) {
    if (lst_p->is_close_nodes(head, head->next)) {
      ...
      Descriptor *descriptor_addr =
          Descriptor::find_descriptor_addr(static_cast<int *>(server_p), p);
      if (descriptor_addr != nullptr) {
        Node tmp = *head;
        std::memmove(head, descriptor_addr->ptr, descriptor_addr->size);
        descriptor_addr->ptr = head;
        head = reinterpret_cast<Node *>(
            static_cast<uint8_t *>(descriptor_addr->ptr) +
            descriptor_addr->size);
        *head = tmp;
      } else {
        return;
      }
  ...
  while (descriptor_addr != nullptr) {
    ...
    descriptor_addr->ptr = head;
    head = reinterpret_cast<Node *>(
        static_cast<uint8_t *>(descriptor_addr->ptr) + descriptor_addr->size);
    *head = tmp;
